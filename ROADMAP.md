...
- [ ] **#186** Seam API hardening before stabilization — **near-term**. Source-span tokens made backing storage observable (`CstToken::source()`) and added a trusted parser-owned reuse hook (`EventBuffer::push_reuse_node_at`). Before treating seam as stable, decide whether to hide/rename these sharp APIs (e.g. internal/unsafe/rebase-specific naming) so public users consume token content rather than backing-buffer ownership or parser-internal reuse machinery.
- [ ] **#187** Recover incremental reuse performance after #61 — **near-term, benchmark first**. Rebased reuse is safe but rebuilds reused token/node structure, regressing edit-only and edit-session benchmarks. Codex PR #188 review also identified downstream physical-identity consumers (e.g. canopy's flat projection changed-index fast path) that need regression coverage before a parent-repo submodule bump. Measure the new cost against the pre-#61 direct-attachment baseline, then design a safe fast path that keeps current-source-backed token views without retaining old source buffers or trusting stale metadata.
