...
|---|----------|------|------------------------|
| CP1 | Per-language authoring cost has scaled past the prior deferral trigger | Scalability friction | Languages went 3 → 6 (`lambda, json, markdown, moonbit, graph-dsl, json-settings`). The 2026-04-19 doc deferred typed-view codegen "until a 4th language quantifies pain" — that trigger is now exceeded. `lambda/src/views.mbt` grew 514 → **644 lines** of cast-and-extract. Examples have **no canonical internal shape**: `lambda` is 9 sub-packages; `markdown` is a flat **30-file / 4 435-LOC** single package. |
| CP2 | `loom/src/core` is the system's accretion point | Boundary / cognitive-load | `core` is **14 299 non-test LOC across 32 source files** spanning ≥6 unrelated subsystems. Churn: **153 changes / 250 commits** (vs `incremental/` = 5). Grammar combinators (`ParserContext::at_adjacent`, `separated_list`) are defined **inside the engine file** `core/parser.mbt` — every language that needs a new boundary/recovery primitive grows the engine package. |
...
| `loom/src/core/` | Edit model, diagnostics, lexer infra, **parser engine + combinators**, reuse, **projection-identity** | **14 299 / 32 files** | **153** |
...
| `examples/*` | 6 grammars + their projections | lambda 10 056; markdown 4 435; graph-dsl 2 004; json 1 703; moonbit 1 541; json-settings 432 | **547** |
...
Co-change analysis (`git log --name-only`, how often each `core` file appears in the *same commit* as the engine file `parser.mbt`):
...
**AP3 — Grammar combinators accrete into the engine file.**
`ParserContext::at_adjacent`/`separated_list` live in `core/parser.mbt`. Because they are *genuinely shared* across languages, accreting them into shared infrastructure is arguably *correct* — but it means `core/parser.mbt` (1 474 LOC, highest core churn) is simultaneously the incremental engine and the grammar-author DSL surface. MoonBit's orphan rule pins these methods to `ParserContext`'s package, so they cannot be cleanly relocated. **Judgment:** this is a watch-item, not a problem to fix now.

**AP4 — `core` is a 6-subsystem package.**
Edit model · diagnostics · lexer infra · engine+reuse · projection-identity in one 14k-LOC unit. Given the §2 low-coupling evidence, a broad split would reduce *navigability* but not *blast radius* — there is no observed "changed X, broke Y." **Judgment:** a full split is largely cosmetic and not currently warranted; only the AP1 extraction is justified by the dependency graph.
...
**Where understanding stops:** I have not read the full bodies of `core/parser.mbt` (1 474 LOC) or `seam/event.mbt` (1 106 LOC); the diagnosis treats the engine as a black box on the strength of its stability evidence (frozen `incremental/`, green differential oracle, independent core churn). That is sufficient to conclude "do not restructure the engine" but **not** sufficient to propose any engine-internal change — and none is proposed.
