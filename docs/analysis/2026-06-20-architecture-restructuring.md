...
- **`@core`-surface caveat (not a pure relocation at package granularity).** The moved symbols (`ProjectionIdentityBaseline`/`ProjectionIdentityTracker`/`ProjectionStringIdAllocator`/`realign_projection_*` — verified present in `loom/src/core/pkg.generated.mbti`) **leave `@core`'s public `.mbti`** and appear in `@projection`'s. The facade re-export preserves all `@loom.*` consumers — which is *every in-repo consumer* (`graph-dsl`/`json-settings` import `@loom.ProjectionIdentityTracker`/`@loom.ProjectionStringIdAllocator`, verified; none import `@core.*` directly) — but a **direct `@core.ProjectionIdentity*` importer** (an external downstream of the published `dowdiness/loom/core` package) would break. So "Stays the same" holds for the **facade** surface, not the `@core` package surface. Mitigation: declare the `@core`-surface reduction in the PR/CHANGELOG, or keep a one-release `#deprecated` re-export of the moved symbols in `@core` if external `@core` compatibility is required. (The target architecture already wants consumers on the facade, so the break is directionally intended — but it must be stated, not folded into "relocation only.")
- **Correctness:** `moon check --deny-warn` + `moon test` per touched module; `find-references` re-confirms no engine consumer; `moon info` → `.mbti` diff shows the symbols **moved from `@core` to `@projection`** (cross-package relocation, not a within-`@core` no-op) + facade re-export repointed; `cd examples/lambda && moon bench --release` shows no regression (lambda doesn't use projection — a deliberate null-check).
...
| Facade (`@loom`) API unchanged by A1; **`@core` surface shrinks** (see Stage A1 `@core`-surface caveat) | `moon info` → `git diff loom/src/**/*.mbti` shows the symbols **moved from `@core` to `@projection`** (cross-package relocation — `@core` loses them, `@projection` gains them — NOT a within-`@core` no-op) + facade re-export repointed; no signature change |
