...
```
BlockReparseSpec::new(
  is_reparseable : (RawKind) -> Bool,
  get_reparser : (SyntaxNode, String, Array[TokenInfo[T]])
    -> ((ParserContext[T, K]) -> Unit)?,
  is_balanced : (Array[TokenInfo[T]]) -> Bool,
  may_reparse_boundary? : (SyntaxNode, Edit, StringView) -> Bool,
)
```
...
**`may_reparse_boundary(node, edit, block_text)`** — explicit grammar-owned admission for candidates that are not strictly interior. Core passes a zero-copy final candidate `StringView` and calls it before detached lexing. False is always safe and preserves fallback cost. True is only an initial gate: detached spans, exact EOF sentinel identity and placement, token balance, parser selection, isolated parsing, exact consumption, and splice validation must still succeed.
...
**`reparse_block(tree, edit, source, spec)`** — orchestrates strict and grammar-admitted candidates: find node → extract text → boundary admission when required → re-lex → validate detached span ordering, bounds, and exact final EOF sentinel → integrity check → reparse → validate complete token consumption and one same-kind replacement node → require full candidate text coverage for boundary candidates → splice. The span validation and text-length proof together reject equal-length overlap/gap cancellation without reconstructing a second string. Candidate admission remains explicit inside core: a selected strict-interior failure preserves the established full-fallback rule, while failure of an additional boundary candidate resumes at the first enclosing strict-interior candidate that existed before boundary admission. Strict-interior candidates retain support for fully consumed sparse token streams with explicit starts. Returns `None` to fall through.
