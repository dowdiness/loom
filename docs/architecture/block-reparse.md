...
BlockReparseSpec::new(
  ...
  owns_boundary_edit? : (SyntaxNode, Edit, StringView) -> Bool,
)
```

`BlockReparseSpec` is opaque. Construct it with `BlockReparseSpec::new`; omitted `owns_boundary_edit` rejects every non-strict candidate. This makes strict-interior reparsing the safe default and lets core evolve its orchestration without exposing record fields.
