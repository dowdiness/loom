...
- [decisions/2026-07-09-parser-driven-lex-mode-api.md](decisions/2026-07-09-parser-driven-lex-mode-api.md) — **Accepted** parser-driven `lex_mode()` / `set_lex_mode()` API for `ParserContext` with `Int`-erased mode; eager tokenization makes parser-driven switching structurally impossible — integration is part of parser-directed memoized tokenization design (tracked in #657), not a standalone wiring task
- [decisions/2026-07-03-structural-ast-testing-grammar-emitter.md](decisions/2026-07-03-structural-ast-testing-grammar-emitter.md) — **Accepted** extract `emit_grammar_module` → `MbtModule` AST to test the grammar emitter structurally via `derive(Eq)` instead of fragile `source.contains` string matching; fixture regeneration requires `moon fmt` post-processing
- [decisions/2026-06-22-grammar-incremental-throughput-gate.md](decisions/2026-06-22-grammar-incremental-throughput-gate.md) — **Accepted** `@grammar` #444 throughput gate: the tree-walking interpreter is at incremental parity on the flat/common path (B/A ≈ 0.9–1.0×) but has a deep-subtree reuse gap (B re-parses deep nested subtrees instead of reusing them → B/A ≈ 1.15–2.4× on deep edits) — graduate the interpreter, defer the code-emitter as the named fix for that gap (#449), keep `@grammar` unblessed until a non-spike consumer
...
- [superpowers/plans/2026-06-29-fold-term-algebra-printers.md](superpowers/plans/2026-06-29-fold-term-algebra-printers.md) — implementation plan for migrating the lambda example's two hand-recursing printers onto the `interpret` catamorphism: add minimal-paren `SourceText`, always-brace `mod` (round-trip-safe via `consume_delimiters`), delete both hand-recursion clusters, add a nested-module positive-control test, retire test-only `Pretty`; 3 tasks, exact snapshot churn enumerated
