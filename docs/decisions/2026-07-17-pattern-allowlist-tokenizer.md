...
Use one package-private cursor parser with a private pattern IR for both annotations. The parser owns allowlisted atoms, escapes, classes, POSIX names, delimiters, quantifiers, and error precedence. Validation consumes the IR for nullability and rejects unsupported or malformed syntax before lexer generation.
