...
**Interfaces:** Define private `MigrationOwnership`, `MigrationWord`, `MigrationMode`, and `MigrationCase`; byte-exact test-only skeleton templates; independent existing/expected renderers; one checker for exact migration, idempotence, deduplication, reversal, and rotation equivalence.

- [ ] Add the eight ownership states and exact state renderings, including `Absent`, without calling production naming or emission helpers.
- [ ] Build identifiers from an equal-width atomic-word catalog, take distinct first words without replacement, and assert unique mode/function/helper names; no retry loop.
...
- [ ] Add basis cases covering every selected ownership state, an unselected canonical state, empty/single/multi/reordered/duplicate selection, identifier depths 1–3, and mode counts 1 and 7.
...
- [ ] Generate 1–7 unique modes by shuffling the atomic-word catalog for distinct first components, then choose identifier depth, trailing components, and ownership independently from `RandomState`.
...
- [ ] Append `@quickcheck.samples(96)` for ordinary variation; separately invoke `Arbitrary` with documented fixed SplitMix seeds covering multiple ownership states, identifier depths 1–3, and multiple selection shapes.
...
- [ ] Obtain an independent exact-head review covering generator independence, oracle independence, all eight ownership states, selection coverage, and exact full-string assertions before PR creation.
